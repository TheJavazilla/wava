...
// TODO WAVA import wava.sun.reflect.misc.ReflectUtil;
...
public class PropertyDescriptor extends FeatureDescriptor {
     ...
     * @param beanClass The Class object for the target bean. For example wava.sun.beans.OurButton.class.
     ...
     * @param propertyName The programmatic name of the property.
     * @param beanClass The Class object for the target bean. For example wava.sun.beans.OurButton.class.
     * @param readMethodName The name of the method used for reading the property value.  May be null if the property is write-only.
     * @param writeMethodName The name of the method used for writing the property value.  May be null if the property is read-only.
     * @exception IntrospectionException if an exception occurs during introspection.
     */
    public PropertyDescriptor(String propertyName, Class<?> beanClass, String readMethodName, String writeMethodName) throws IntrospectionException {
        ...
        if (readMethodName != null && getReadMethod() == null) throw new IntrospectionException("Method not found: " + readMethodName);

        this.writeMethodName = writeMethodName;
        if (writeMethodName != null && getWriteMethod() == null) throw new IntrospectionException("Method not found: " + writeMethodName);

        // If this class or one of its base classes allow PropertyChangeListener,then we assume that any properties we discover are "bound".
     ...
     *
     * @param propertyName The programmatic name of the property.
     * @param readMethod The method used for reading the property value. May be null if the property is write-only.
     * @param writeMethod The method used for writing the property value. May be null if the property is read-only.
     * @exception IntrospectionException if an exception occurs during introspection.
     */
    public PropertyDescriptor(String propertyName, Method readMethod, Method writeMethod) throws IntrospectionException {
        if (propertyName == null || propertyName.length() == 0) throw new IntrospectionException("bad property name");
    ...
    PropertyDescriptor(Class<?> bean, String base, Method read, Method write) throws IntrospectionException {
        if (bean == null) throw new IntrospectionException("Target Bean class is null");
     ...
     * Note that the {@code Class} object may describe primitive Java types such as {@code int}.
     * This type is returned by the read method or is used as the parameter type of the write method.
     * Returns {@code null} if the type is an indexed property that does not support non-indexed access.
     *
     * @return the {@code Class} object that represents the Java type info, or {@code null} if the type cannot be determined
     */
    public synchronized Class<?> getPropertyType() {
        ...
        if (type  == null) {
            ...
            } catch (IntrospectionException ex){/*Fall*/}
        }
        return type;
    }
    ...
    public synchronized Method getReadMethod() {
        ...
        if (readMethod == null) {
            ...
            if (cls == null || (readMethodName == null && !this.readMethodRef.isSet()))
                return null; // The read method was explicitly set to null.

            String nextMethodName = Introspector.GET_PREFIX + getBaseName();
            if (readMethodName == null) {
                Class<?> type = getPropertyType0();
                if (type == boolean.class || type == null) readMethodName = Introspector.IS_PREFIX + getBaseName();
                else readMethodName = nextMethodName;
            }

            // Since there can be multiple write methods but only one getter method, find the getter method first so that you know what the
            // property type is.  For booleans, there can be "is" and "get" methods. If an "is" method exists, this is the official reader method so look for this one first.
    ...
    public synchronized Method getWriteMethod() {
        ...
        if (writeMethod == null) {
            ...
            if (cls == null || (writeMethodName == null && !this.writeMethodRef.isSet()))
                return null; // The write method was explicitly set to null.
            ...
            if (type == null) {
                try {
                    ...
                    // Without the correct property type we can't be guaranteed to find the correct method.
                    return null;
                }
            }
            ...
            if (writeMethod != null)
                if (!writeMethod.getReturnType().equals(void.class)) writeMethod = null;

            try {
                setWriteMethod(writeMethod);
            } catch (IntrospectionException ex) {/* fall through */}
        }
        return writeMethod;
    }
    ...
    void setClass0(Class<?> clz) {
        if (getClass0() != null && clz.isAssignableFrom(getClass0()))
            return; // don't replace a subclass with a superclass
     ...
     * PropertyEditor with a given property, then you can do it with this method.
     *
     ...
     * for this property.
     *
     * @return Any explicit PropertyEditor Class that has been registered for this property.
     *          Normally this will return "null", indicating that no special editor has been registered, so the PropertyEditorManager should be used to locate a suitable PropertyEditor.
     */
     ...
     * @return a property editor instance or null if a property editor has not been defined or cannot be created
     */
    public PropertyEditor createPropertyEditor(Object bean) {
        ...
        if (cls != null && PropertyEditor.class.isAssignableFrom(cls) /*&& ReflectUtil.isPackageAccessible(cls)*/) {
            ...
            try {
                if (ctor == null) editor = cls.newInstance();
                else editor = ctor.newInstance(new Object[] { bean });
            } catch (Exception ex) {/*Fall through*/}
        }
     ...
     * are the same if the read, write, property types, property editor and flags are equivalent.
     */
    public boolean equals(Object obj) {
        ...
        if (obj != null && obj instanceof PropertyDescriptor) {
            ...
            if (!compareMethods(getReadMethod(), otherReadMethod)) return false;
            if (!compareMethods(getWriteMethod(), otherWriteMethod)) return false;

            if (getPropertyType() == other.getPropertyType() && getPropertyEditorClass() == other.getPropertyEditorClass() && bound == other.isBound() 
                && constrained == other.isConstrained() && writeMethodName == other.writeMethodName && readMethodName == other.readMethodName)
                return true;
        }
        return false;
    }
     ...
     * Merge two property descriptors.  Where they conflict, give the second argument (y) priority over the first argument (x).
     *
    ...
    PropertyDescriptor(PropertyDescriptor x, PropertyDescriptor y) {
        ...
        if (y.baseName != null) baseName = y.baseName;
        else baseName = x.baseName;

        if (y.readMethodName != null) readMethodName = y.readMethodName;
        else readMethodName = x.readMethodName;

        if (y.writeMethodName != null) writeMethodName = y.writeMethodName;
        else writeMethodName = x.writeMethodName;

        if (y.propertyTypeRef != null) propertyTypeRef = y.propertyTypeRef;
        else propertyTypeRef = x.propertyTypeRef;
        ...
        try {
            if (isAssignable(xr, yr)) setReadMethod(yr);
            else setReadMethod(xr);
        } catch (IntrospectionException ex) {
            // fall through
        }
        ...
        try {
            if (yw != null) setWriteMethod(yw);
            else setWriteMethod(xw);
        } catch (IntrospectionException ex){/*Fall through*/}

        if (y.getPropertyEditorClass() != null) setPropertyEditorClass(y.getPropertyEditorClass());
        else setPropertyEditorClass(x.getPropertyEditorClass());
    ...
    void updateGenericsFor(Class<?> type) {
        ...
        } catch (IntrospectionException exception){setPropertyType(null);}
    }
    ...
    private Class<?> findPropertyType(Method readMethod, Method writeMethod)
        ...
        } catch (IntrospectionException ex){throw ex;}
        return propertyType;
    }
