...
import static wava.sun.awt.SunHints.*;
...
public class FileFontStrike extends PhysicalStrike {
    ...
    private float getGlyphAdvance(int glyphCode, boolean getUserAdv) {
        ...
        if (invertDevTx != null || !getUserAdv) {
            ...
            if (false && glyphPtr != 0L) {
                // TODO advance = StrikeCache.unsafe.getFloat(glyphPtr + StrikeCache.xAdvanceOffset);
            } else {
    ...
    void getGlyphImageBounds(int glyphCode, Point2D.Float pt,
        ...
        topLeftX = ptr;//StrikeCache.unsafe.getFloat(ptr+StrikeCache.topLeftXOffset);
        topLeftY = ptr;//StrikeCache.unsafe.getFloat(ptr+StrikeCache.topLeftYOffset);

        result.x = (int)Math.floor(pt.x + topLeftX);
        result.y = (int)Math.floor(pt.y + topLeftY);
        result.width =
            (int) ptr;//StrikeCache.unsafe.getShort(ptr+StrikeCache.widthOffset)  &0x0ffff;
        result.height =
            (int) ptr;//StrikeCache.unsafe.getShort(ptr+StrikeCache.heightOffset) &0x0ffff;
    ...
    GeneralPath getGlyphOutline(int glyphCode, float x, float y) {
        ...
        gp = (GeneralPath)wava.helpers.Cloner.clone(gp); // mutable!
        if (x != 0f || y != 0f) {
