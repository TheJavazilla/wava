package wava.sun.awt.datatransfer;
...
import wava.sun.awt.EventListenerAggregate;
...
public abstract class SunClipboard extends Clipboard
    ...
    public synchronized void removeFlavorListener(FlavorListener listener) {
        ...
        EventListenerAggregate contextFlavorListeners = (EventListenerAggregate) appContext.get(CLIPBOARD_FLAVOR_LISTENER_KEY);
        if (contextFlavorListeners == null)
            return; //else we throw NullPointerException, but it is forbidden
    ...
    public synchronized FlavorListener[] getFlavorListeners() {
        EventListenerAggregate contextFlavorListeners = (EventListenerAggregate) AppContext.getAppContext().get(CLIPBOARD_FLAVOR_LISTENER_KEY);
        return contextFlavorListeners == null ? new FlavorListener[0] : (FlavorListener[])contextFlavorListeners.getListenersCopy();
    }
    ...
    protected final void checkChange(final long[] formats) {
        if (Arrays.equals(formats, currentFormats)) {
            ...
            // DataFlavors this and previous time and they are coincident; don't notify
            return;
        }
