...
import wava.sun.swing.plaf.synth.Paint9Painter;
...
class ImagePainter extends SynthPainter {
    ...
    ImagePainter(boolean tiles, boolean paintCenter,
        ...
        if (sourceInsets != null) {
            this.sInsets = (Insets)wava.helpers.Cloner.clone(sourceInsets);
        }
        if (destinationInsets == null) {
            dInsets = sInsets;
        } else {
            this.dInsets = (Insets)wava.helpers.Cloner.clone(destinationInsets);
        }
        this.tiles = tiles;
    ...
    public Insets getInsets(Insets insets) {
        if (insets == null) {
            return (Insets)wava.helpers.Cloner.clone(this.dInsets);
        }
