...
public class ShowPanel extends JPanel {
    ...
    public ShowPanel(String s, Show show) {
        ...
        for (String fz : show.getEpisodes()) {
            ...
            String vid = path.contains("embed") ? path.substring(path.indexOf("embed") + "embed".length() + 1) : path.substring(path.indexOf("?v=") + "?v=".length());

            ((Button)l.gwt).addClickHandler(new ClickHandler() {
                public void onClick(ClickEvent event) {playWeb(vid);}
            });
            l.setSize(850, 90);
    ...
    public void playWeb(String vid) {
        String path = "https://www.youtube.com/embed/" + vid + "?autoplay=1";
